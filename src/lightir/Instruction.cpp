...
void BinaryInst::assertValid()
{
    ...
    assert(static_cast<IntegerType *>(get_operand(0)->get_type())->get_num_bits() == static_cast<IntegerType *>(get_operand(1)->get_type())->get_num_bits());
}
...
std::string BinaryInst::print()
{
    ...
    instr_ir += " ";
    instr_ir += this->get_operand(0)->get_print_type()->print();
    instr_ir += " ";
    instr_ir += print_as_op(this->get_operand(0), false);
    instr_ir += ", ";
    if (Type::is_eq_type(this->get_operand(0)->get_print_type(), this->get_operand(1)->get_print_type()))
    {
...
std::string CallInst::print()
{
    ...
    for (int i = 1; i < this->get_num_operand(); i++)
    {
        if (i > 1)
            instr_ir += ", ";
        instr_ir += this->get_operand(i)->get_print_type()->print();
        instr_ir += " ";
...
Type *GetElementPtrInst::get_element_type(Value *ptr, std::vector<Value *> idxs)
{
    ...
    else if (ty->is_struct_type())
    {
        ...
        auto struct_type = dynamic_cast<StructType *>(ty);
        ty = struct_type->get_members()[element_idx].type;
    }
    return ty;
}
...
std::string GetElementPtrInst::print()
{
    ...
    instr_ir += this->get_module()->get_instr_op_name(this->get_instr_type());
    instr_ir += " ";
    assert(this->get_operand(0)->get_print_type()->is_pointer_type());
    instr_ir += this->get_operand(0)->get_print_type()->get_pointer_element_type()->print();
    instr_ir += ", ";
