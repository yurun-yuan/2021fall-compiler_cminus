...
#define CASE(s) else if (_STR_EQ((n->name), s))
...
AST::transform_node_iter(syntax_tree_node *n)
{
    if (_STR_EQ(n->name, "program"))
    {
        ...
        while (!s.empty())
        {
            auto child_node = static_cast<ASTDefinition *>(
                transform_node_iter(s.top()));

            auto child_node_shared =
                std::shared_ptr<ASTDefinition>(child_node);
            node->definitions.push_back(child_node_shared);
            s.pop();
        }
        return node;
    }
    else if (_STR_EQ(n->name, "definition"))
    {
        return transform_node_iter(n->children[0]);
    }
    else if (_STR_EQ(n->name, "var-definition"))
    {
        auto node = new ASTVarDefinition();
        ...
        {
            std::cerr << "[ast]: var-definition transform failure!" << std::endl;
            std::abort();
        }
        return node;
    }
    else if (_STR_EQ(n->name, "fun-definition"))
    {
        auto node = new ASTFunDefinition();
        if (_STR_EQ(n->children[0]->children[0]->name, "int"))
        {
            node->type = TYPE_INT;
        }
    ...
    else if (_STR_EQ(n->name, "compound-stmt"))
    {
        ...
        if (n->children[1]->children_num == 2)
        {
            ...
            {
                auto decl_node =
                    static_cast<ASTVarDefinition *>(
                        transform_node_iter(s.top()));
                auto decl_node_ptr = std::shared_ptr<ASTVarDefinition>(decl_node);
    ...
    else if (_STR_EQ(n->name, "additive-expression"))
    {
        ...
        if (n->children_num == 3)
        {
            ...
            auto term_node =
                static_cast<ASTMultiplicativeExpression *>(
                    transform_node_iter(n->children[2]));
            node->multiplicative_expression = std::shared_ptr<ASTMultiplicativeExpression>(term_node);
        }
        else
        {
            auto term_node =
    ...
    else if (_STR_EQ(n->name, "multiplicative-expression"))
    {
        auto node = new ASTMultiplicativeExpression();
        if (n->children_num == 3)
        {
            auto term_node =
                static_cast<ASTMultiplicativeExpression *>(
            ...
            node->multiplicative_expression =
                std::shared_ptr<ASTMultiplicativeExpression>(term_node);
...
void ASTPrinter::visit(ASTVarDefinition &node)
{
    _DEBUG_PRINT_N_(depth);
    std::cout << "var-definition: " << node.id;
    if (node.num != nullptr)
    {
        std::cout << "[]" << std::endl;
        add_depth();
...
void ASTPrinter::visit(ASTFunDefinition &node)
{
    _DEBUG_PRINT_N_(depth);
    std::cout << "fun-definition: " << node.id << std::endl;
    add_depth();
...
void ASTPrinter::visit(ASTMultiplicativeExpression &node)
{
    _DEBUG_PRINT_N_(depth);
    std::cout << "multiplicative-expression";
    if (node.multiplicative_expression == nullptr)
    {
        std::cout << std::endl;
    }
    else
